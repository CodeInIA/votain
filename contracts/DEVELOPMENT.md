...
|----------|-------------|
| ElectionV4.sol | Single election. Members tree (LeanIMT/PoseidonT3), `enroll` gated to PlatformRegistry members plus an optional attribute policy (`enrollAttested`), `enrollPrivate` with a `documentTag` on gated elections. `castVote(BallotInput, Proof)`: an ElGamal vote and a cancellation of the voter's previous ballot, proved by the ballot circuit; checks the members and ballots roots, the ballot's `tag` (once) and `epochTag` (one per voter per `EPOCH_LENGTH`), files the ballot's leaf in the ballots tree and adds it to the on-chain aggregate. `publishResults(cid, counts, Proof)` verifies the tally circuit's proof against that aggregate; `voidBelowQuorum(voters, Proof)` ends an election below its quorum without revealing counts; `markVoided` is refused on a non-cancellable election with as many ballots as its quorum. The tag scope derives from the contract's own address. `VotingType` + `thresholdValue`, lifecycle (cancel / open and close early / void / publish). No meta-transaction forwarder. |
| ElectionFactory.sol | Creates elections through `ElectionDeployer` from a `Config` struct, choosing the smallest verifier pair (`verifiersFor`) that holds the options plus the blank vote; routes the deposit to the paymaster; enumerable `getElections(offset, limit)`. |
| ElectionDeployer.sol | Holds ElectionV4's creation code, which no longer fits inside the factory under the 24576-byte limit. Stateless and permissionless: only the factory can register what it deploys with the paymaster. |
| ElectionPaymaster.sol | Gas tank **and relay hub**. `relayEnroll` / `relayEnrollAttested` / `relayEnrollPrivate` / `relayVote` call the election and reimburse the caller from the election's reserve, then the organizer's free balance, in the same tx. Keeps no per-voter state: the election's epoch tags bound the rate. `deposit` / `depositForElection` / `withdraw`; `setFactory` is onlyOwner and settable ONCE; `setRelayParams` is onlyOwner within fixed ceilings. Ownership is `TwoStepOwnable`, shared with `PlatformRegistry`. |
...
| BabyJubJub.sol | Point addition in extended twisted Edwards coordinates, for the aggregate. |
| IBallotVerifier.sol, ITallyVerifier.sol | What an election calls: `slots()` and a verify taking the public signals as a dynamic array. |
| verifiers/*.sol | GENERATED by `circuits/scripts/build.mjs` (`BallotVerifierS<N>`, `TallyVerifierS<N>`), never committed. Build the circuits before compiling. |
| mocks/MockVerifier.sol | `MockBallotVerifier` / `MockTallyVerifier`: accept any proof with the right number of signals, unit tests and circuit-less local runs only. |

**External libraries**: `PoseidonT3` (the trees) and `PoseidonT4` (the keys hash) from
poseidon-solidity are linked into ElectionV4, and so into `ElectionDeployer`. The deploy script
deploys them deterministically (CREATE2) so they land at the same address on every chain.
...
ALLOW_INSECURE_CEREMONY=1  # only for a throwaway testnet: deploy verifiers from the public
                           # development ceremony, which deploy.ts otherwise refuses off-chain-local
```
