...
`phase()`), **in-app tally** with tally keys **derived from the organizer's wallet signature**
...
**Done since this list was written**, verified against the tree on 2026-09-18 and updated on
2026-09-24: `deploy.ts` deploys the ballot and tally verifiers generated by `circuits/` (mock
verifiers only on a local chain with no circuits built, and a development ceremony is refused
off the local chain unless `ALLOW_INSECURE_CEREMONY=1`); `cancelElection`, `closeEnrollmentEarly`,
...
- Code-splitting lazy-load for Semaphore WASM and snarkjs (the proving files are fetched only when a ballot is cast).
...
| Hidden re-votes | 2026-09-24 | Ballots moved from Paillier to exponential ElGamal on Baby Jubjub, one key per option, proved by a new ballot circuit (`circuits/`). Every ballot carries a vote and a cancellation of the voter's previous ballot, proved against a ballots tree without saying which, and is filed under a per-step tag, so a first ballot and a re-vote look alike and share nothing public. `ElectionV4` keeps the aggregate itself and publishes a result only with the tally circuit's proof of its decryption; below the quorum `voidBelowQuorum` proves only that too few voted. Epoch tags replace the per-nullifier re-vote cooldown. `ElectionDeployer` split off for code size. ABI parity test over every hand-written fragment, which found a local-relay signature left behind by the `documentTag` change |
