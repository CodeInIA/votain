/**
 * The words a recovery phrase is built from: the EFF Short Wordlist 1.
 *
 * Designed by the Electronic Frontier Foundation for exactly this job: words a
 * person reads off a screen and types back. Every word is three to five letters,
 * common, easy to spell, and free of homophones and offensive entries. The one
 * entry with a hyphen, "yo-yo", is left out, so a phrase is always plain
 * letters separated by spaces. 1295 words.
 *
 * Source: https://www.eff.org/dice (EFF Short Wordlist 1, CC BY 3.0 US), taken
 * from the @wordlist/english-eff package. Changing this list changes which
 * phrases are valid, so it is never edited in place.
 */
export const EFF_SHORT_WORDS: readonly string[] = [
