...
import { Fingerprint, ShieldAlert, CheckCircle, ChevronRight, ChevronLeft, ShieldCheck, Loader2 } from 'lucide-react';
...
export default function Onboarding() {
  ...
  const handleOpenWorldId = async () => {
    try {
      const response = await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/rp-signature`, {
         method: 'POST',
      ...
      console.error("Failed fetching rp_context sig", e);
      setIsOpen(true);
    }
  };
  ...
  const handleVerify = async (proof: any) => {
    ...
    try {
      ...
      // Send the proof generated by IDKit to our backend for verification
      // with Worldcoin servers and to return the credential (SD-JWT)
      ...
      // Store the credential locally to maintain a persistent session
      if (data.sdJwt) {
  ...
  const onSuccess = () => {
    // This is called when the IDKit modal is successfully closed
    navigate('/voter/dashboard');
  };
  ...
  const stepsData = [
    ...
    },
    {
      ...
      icon: <img src="/world-id-logo.svg" alt="World ID" className="w-16 h-16 sm:w-20 sm:h-20" style={{ filter: "invert(1) brightness(200%)" }} />
    }
  ];
                  ...
                  <div style={{ display: "none" }}>
                    {rpContext && (
                      <IDKitRequestWidget
                        ...
                        app_id={import.meta.env.VITE_WORLD_ID_APP_ID as `app_${string}`}
                        action={import.meta.env.VITE_WORLD_ID_ACTION}
                        rp_context={rpContext}
                        allow_legacy_proofs={true}
                        preset={{ type: "DeviceLegacy" }}
