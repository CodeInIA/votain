...
describe('Voter Onboarding Flow', () => {
  ...
  it('allows navigation via step indicators', async () => {
    ...
    // Check for the 4 indicator buttons mapping to the steps
  ...
  it('navigates to /voter/verify on the final step', async () => {
    setup();
    
    // Click to the last step (4th step) using indicators
    const indicators = screen.getAllByRole('button', { name: /Go to step/i });
    fireEvent.click(indicators[3]);
    
    // The button should now be "Start Verification" instead of "Next"
    const startBtn = await screen.findByRole('button', { name: 'onboarding.btn_start' });
    expect(startBtn).toBeInTheDocument();
    
    // Click to finish onboarding
    fireEvent.click(startBtn);
    
    // It should navigate to the next feature
    expect(mockNavigate).toHaveBeenCalledWith('/voter/verify');
  });
});
